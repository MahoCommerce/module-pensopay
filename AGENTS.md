...
### Routing, Observers & Cron (PHP attributes)

Requires Maho **26.5+**. Routes, event observers, and cron jobs are declared via PHP attributes on the relevant methods — **not** XML in `config.xml`:

- Routes: `#[\Maho\Config\Route('/path', name: '...')]` on controller `*Action()` methods
- Observers: `#[\Maho\Config\Observer('event_name', area: '...', type: '...', id: '...')]` on `Model/Observer.php` methods
- Cron: `#[\Maho\Config\CronJob('id', schedule: '...')]` on `Model/Observer.php` methods

After editing any attribute, run `composer dump-autoload` to recompile `vendor/composer/maho_*.php`. The `<admin><routers>` registration chain remains in `config.xml`.
